...
function modalShow(a) {
  ...
  if (person[a].traits !== undefined)
    person[a].traits.forEach(
      (item) => (traits.innerHTML += `<li>${item}</li>`)
    );
  traits.addEventListener('mouseleave', () =>
    traits.classList.remove('drag-cursor')
  );
  if (traits.offsetWidth < traits.scrollWidth) {
    traits.addEventListener('mousedown', () =>
      traits.classList.add('drag-cursor')
    );
    traits.addEventListener('mouseup', () =>
      traits.classList.remove('drag-cursor')
    );
  }
